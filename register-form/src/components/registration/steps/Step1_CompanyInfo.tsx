...
const Step1CompanyInfo: React.FC<Step1CompanyInfoProps> = ({ legalForms, departments }) => {
  ...
  const watchedDepartments = watch('departamentos');
  const departmentsValue = useMemo(() => watchedDepartments ?? [], [watchedDepartments]);
        ...
        <div className="md:col-span-2">
          ...
          <div className="space-y-2">
            {departmentsValue.map((_, index) => (
              <div key={`departamento-${index}`} className="flex gap-3 items-center">
                <Select
                  ...
                  error={errors.departamentos?.[index]?.message}
                >
                  <option value="">Seleccione...</option>
            ...
            <option
              key={id}
              value={String(id)}
              disabled={departmentsValue.some((d, i) => i !== index && String(d) === String(id))}
            >
              {nombre}
            </option>
                  ))}

/**
 * Simulación de registro de empresa al máximo capacity - VERSIÓN STANDALONE
 * Este script emula el llenado completo del formulario de empresas
 * respetando los límites de la UI (sin dependencias externas)
 */

// ============================================================================
// TIPOS (copiados del proyecto para ser standalone)
// ============================================================================
...
type ComunidadIndigenaDto = NamedResource;

type MunicipioDto = NamedResource & {
  comunidadesIndigenas: ComunidadIndigenaDto[];
};

type DepartamentoDto = NamedResource & {
  municipios: MunicipioDto[];
};

type ProjectMunicipioTrabajoForm = {
  idMunicipio: string;
  idComunidadIndigena?: string;
};

type ProjectUbicacionForm = {
  departamento: string;
  municipiosTrabajo: ProjectMunicipioTrabajoForm[];
};

type ProjectAyudaForm = {
  seleccionados: string[];
  otros: string[];
};

type ProjectConservacionForm = {
  especies: ProjectAyudaForm;
  practicasAgricolas: ProjectAyudaForm;
};

type ProjectDesarrolloForm = {
  seleccionados: string[];
  otros: string[];
};

type ProjectTipoForm = {
  id?: string;
  otros?: string;
};
...
function buildMaxProject(index: number, currentYear: number, orgIds: string[]): ProjectFormData {
  ...
  const proyecto: ProjectFormData = {
    ...
    fechaFin: index % 3 === 0 ? '2025-12-31' : undefined,
    ...
    ubicaciones,
    ayudas: {
      seleccionados: mockHelpTypes.map(h => String(h.id)),
      otros: [
    ...
    actores: {
      seleccionados: mockLocalActors.map(a => String(a.id)),
      otros: [
    ...
    conservacion: {
      especies: {
        seleccionados: mockSpecies.map(s => String(s.id)),
        otros: [
          'Oso andino',
          'Loro militar',
          'Nutria gigante',
      ...
      practicasAgricolas: {
        seleccionados: mockAgriculturalPractices.map(p => String(p.id)),
        otros: [
    ...
    desarrollo: {
      seleccionados: mockDevelopmentAreas.map(da => String(da.id)),
      otros: [
    ...
    organizacionesRelacionadas: orgIds,
  };
...
const sizeInfo = calculateSize(maxForm);
...
// 6. Mostrar JSON completo (primera parte)
console.log('📋 MUESTRA DEL JSON GENERADO (primeros 2000 caracteres):\n');
const fullJson = JSON.stringify(maxForm, null, 2);
...
const outputPath = path.join(outputDir, 'max-company-registration.json');
fs.writeFileSync(outputPath, fullJson, 'utf-8');

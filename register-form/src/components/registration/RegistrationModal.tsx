...
import { registerFormularioEmpresaSchema } from '../../types/api.schema';
...
const steps: { id: number; name: string; fields: Path<RegistrationFormData>[] }[] = [
...
const RegistrationModal: React.FC<RegistrationModalProps> = ({ onClose }) => {
  ...
  const { submit: onSubmit, isSubmitting, isSuccess, submitError, submitTechnical, successMessage } = useFormSubmission({
    methods,
    mapToDto: mapEmpresaFormToDto,
    dtoSchema: registerFormularioEmpresaSchema,
    endpoint: '/api/formularios/empresas',
    clearDraft,
    onSuccess: () => setCurrentStep(0),
    errorLogLabel: 'formulario de empresas',
  });
            ...
            >
              ...
              <div className="px-6 pt-8 pb-6 bg-gray-50 dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center rounded-b-2xl">
                ...
                {currentStep < totalSteps - 1 ? (
                  <Button type="button" variant="primary" onClick={() => { void nextStep(); }}>
                    Siguiente
                  </Button>
                ) : (
                  <Button
                    ...
                    onClick={canSubmit ? () => { void methods.handleSubmit(onSubmit)(); } : undefined}
                  >

...
export const useDayNightCycle = (updateInterval: number = 60000) => {
  ...
  const setTheme = useCallback((newTheme: 'day' | 'night') => {
    const root = window.document.documentElement;
    if (newTheme === 'night') {
      root.classList.add('dark');
      setCurrentTheme(TIME_THEMES.night);
    } else {
      root.classList.remove('dark');
      setCurrentTheme(TIME_THEMES.day);
    }
  ...
  useEffect(() => {
    ...
    const interval = setInterval(updateTheme, updateInterval);

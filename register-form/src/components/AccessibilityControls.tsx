...
const isPartialAccessibilitySettings = (value: unknown): value is Partial<AccessibilitySettings> => (
...
const AccessibilityControls: React.FC<AccessibilityControlsProps> = ({
  ...
  const { toggle: toggleAudio, setVolume: setAudioVolume } = useSoundManager();
  ...
  const handleReducedMotionToggle = useCallback((enabled: boolean) => {
    ...
    saveSettings({ reducedMotion: enabled, effectsEnabled, volume });
  ...
  }, [effectsEnabled, volume, onReducedMotionChange, saveSettings]);
  ...
  useEffect(() => {
    ...
    if (savedSettings) {
      try {
        ...
        if (isPartialAccessibilitySettings(settings)) {
